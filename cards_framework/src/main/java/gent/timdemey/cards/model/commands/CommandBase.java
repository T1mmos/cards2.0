...
{   	
    ...
    protected final void CheckNotContext(ContextType current, ContextType ... types)
    {
        for (ContextType type : types)
        {
            if (current == type)
            {
                throw new IllegalStateException("This command cannot be executed under the current context type " + current);
            }
        }
    }
    ...
    public String getSerialized()
    {
        if (serialized == null)
        {
            throw new IllegalStateException("No serialized form of this command was set, this can happen when the command did not enter via a TCP connection.");
        }
        return serialized;
    }
}

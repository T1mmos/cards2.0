 ...
 *
 */
public class C_StartServer extends CommandBase<P_StartServer>
{
    ...
    public void execute()
    {
        if(_ContextType == ContextType.UI)
        {
            _Starter.startServer((P_StartServer) _Payload);
        }
        else
        {
            try
            {
                ...
                // create a player             
                Player player = _StateFactory.CreatePlayer(creatorId, playerName);
                
                // create web service to announce the presence over UDP 
                UDP_ServiceResponder udpServAnnouncer = _NetworkFactory.CreateUDPServiceAnnouncer(udpport);
    ...
    private class UDPListener implements IUdpMessageListener
    {
        ...
        {
            _Logger.info("Received some data, trying to parse it as a C_UDP_Request...");
            ...
            if (!(command instanceof C_UDP_GetServerInfoRequest))
            {
                _Logger.error("Ignoring the received data as the command is not a '" + C_UDP_GetServerInfoRequest.class.getSimpleName() + "' but is of type '" + command.getClass().getSimpleName() + "'");
                return;
            }

            // schedule the UDP request command
            C_UDP_GetServerInfoRequest udpRequestCmd = (C_UDP_GetServerInfoRequest) command;
